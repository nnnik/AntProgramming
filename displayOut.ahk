...
class display
{
	...
	*/
	...
	addPicture( file, pos = "", size = "" , rot := "" )
	{
		...
		pic.setRotation( isObject( rot ) ? rot : [ 0, 0, 0 ] )
		pic.setFile( file )
		return pic
	}
	...
	*/
	...
	class Picture
	{
		...
		*/
		...
		setPosition( pos )
		{
			if ( This.getVisible() && This.pos.3 != pos.3 )
				This.joinZLayer()
			This.pos := pos
		}
		...
		/*
			...
			rot:			The rotation of the picture on the field along 3 axes
			Has to be in the form [ rotZ, rotY, rotX ]
			Currently only rotations around the Z axis are supported. 
			You are expected to fill the other 2 fields with 0
	...
	drawZLayer( zLayer )
	{
		...
		if ( zBuffer.hasChanged() || zLayer.hasChanged() )
		{
			...
			For each, pic in pics
			{
				...
				size := Bitmap.getSize()
				sRect := [ 0, 0, size.1, size.2 ]
				tRect := This.fieldToPixel( pic )
				if ( zRotation := pic.getRotation().1 )
					Graphics.rotateCanvasTableAroundPoint( tRect.1 + tRect.3 // 2 , tRect.2 + tRect.4 // 2, zRotation )
				Graphics.drawBitmap( Bitmap, tRect, sRect )
				if ( zRotation )
					Graphics.resetCanvasTable()
			}
		}
	}
